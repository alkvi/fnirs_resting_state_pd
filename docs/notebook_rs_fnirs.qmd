...
tab1 <- tableby(group ~ sex + age + weight + height +
                mb_total + updrs_3_total + led_total, data=all_subject_data, control=mycontrols)
...
hc <- braph_long$value[grepl("^FNP1", braph_long$subject)]

# PD = subjects starting with FNP2
pd <- braph_long$value[grepl("^FNP2", braph_long$subject)]

paste("diff")
mean_hc - mean_pd
cohen.d(hc, pd, hedges.correction = TRUE)
