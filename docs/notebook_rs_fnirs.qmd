...
# Also count how many bads we have
bad_counts <- bad_shorts |>
  mutate(
    n_bad = case_when(
      is.na(bad_shorts) ~ 0,
      bad_shorts == "" ~ 0,
      TRUE ~ str_count(bad_shorts, ",") + 1
    )
  )

summary_stats <- bad_counts |>
  summarise(
    median = median(n_bad),
    q1 = quantile(n_bad, 0.25),
    q3 = quantile(n_bad, 0.75),
    min = min(n_bad),
    max = max(n_bad)
  )
...
summary_stats |>
  gt() |>
  tab_header(
    title = "Bad shorts per session count"
  )
...
# Demographics via gtsummary
# https://www.danieldsjoberg.com/gtsummary/
# Might warn about controls missing PD variables, and ties; ignore this.
...
tab1_demo <-
  tbl_summary(
    all_subject_data,
    include = c(sex, age, weight, height, moca_sum,
                mb_total, laterality_quotient, updrs_3_total, led_total),
    by = group,
    missing = "no",
    digits = list(age ~ 0),
    type = list(
    ...
    statistic = list(
      ...
      # non-parametric
      moca_sum ~ "{median} ({p25}, {p75})",
      mb_total ~ "{median} ({p25}, {p75})",
      laterality_quotient ~ "{median} ({p25}, {p75})",
      led_total ~ "{median} ({p25}, {p75})"
    )
  ) |>
  add_p(
    test = list(
      all_continuous() ~ "t.test",
      moca_sum ~ "wilcox.test",
      mb_total ~ "wilcox.test",
      laterality_quotient ~ "wilcox.test"
    )
  ) |>
  bold_p(t = 0.05) |>
...
We can also use the MDS-UPDRS III to evaluate if one side was more affected than the other.

To do this: calculate scores for items evaluated on each side, and sum them.
...
lat_table <-
  tbl_summary(
    updrs_data,
    by = group,
    include = c(right_total, left_total, asymmetry_total, asymmetry_2pt),
    missing = "no",
    statistic = all_continuous() ~ "{median} ({p25}, {p75})"
  ) |>
  modify_header(label ~ "**Variable**") |>
  modify_caption("**Table - Affected side**")
...
g_res <- hedges_g(hc, pd)

hedges_g <- g_res$Hedges_g
ci_low   <- g_res$CI_low
ci_high  <- g_res$CI_high
...
fig.savefig("../results_figures/markers.pdf", bbox_inches="tight")
...
final_plot <- plot_grid(plotlist=list(plot1, plot2, plot3, plot4), ncol=2, nrow=2)

ggsave(
  filename = "../results_figures/corr.pdf",
  plot = final_plot,
  units = "in"
)
...
From the source code, Core-Periphery is defined such that:
...
- <https://github.com/braph-software/BRAPH-2/blob/develop/braph2/measures/CorePeriphery.m>

This means that the PD group had more instances of this channel belonging to the network core compared to HC.
...
braph_expl_left_subject = "../data/braph_left_exploratory_subject_results.csv"
...
braph_expl_left_subject_data <- read.csv(braph_expl_left_subject)
...
# Effect size 
filtered_braph_expl <- braph_expl_left_subject_data %>%
  filter(ch == "SRC3-DET1") %>%                      # keep desired condition
  select(ch, ends_with("_CorePeriphery"))            # keep only relevant measures

# Reshape braph_left_subject_data to long format for effsize and merging 
# with subject covariates in correlation analyses
braph_long_src_3_det_1 <- filtered_braph_expl %>%
  pivot_longer(cols = starts_with("FNP"), names_to = "subject", values_to = "value")
braph_long_src_3_det_1 <- braph_long_src_3_det_1 %>%
  mutate(subject = gsub("rs", "", subject))

# HC = subjects starting with FNP1
hc <- braph_long_src_3_det_1$value[grepl("^FNP1", braph_long_src_3_det_1$subject)]

# PD = subjects starting with FNP2
pd <- braph_long_src_3_det_1$value[grepl("^FNP2", braph_long_src_3_det_1$subject)]
...
# Contingency table
cont_tab <- table(
  group = c(rep("HC", length(hc)), rep("PD", length(pd))),
  value = c(hc, pd)
)

# Cohen's h
h_res <- cohens_h(cont_tab)
h_val  <- h_res$Cohens_h
ci_low <- h_res$CI_low
ci_high <- h_res$CI_high

# Counts for reporting
hc_n1 <- sum(hc == 1, na.rm = TRUE)
hc_total <- length(hc)
pd_n1 <- sum(pd == 1, na.rm = TRUE)
pd_total <- length(pd)

# Create table
tibble(
  "HC (n, %)" = sprintf("%d/%d (%.1f%%)", hc_n1, hc_total, mean_hc * 100),
  "PD (n, %)" = sprintf("%d/%d (%.1f%%)", pd_n1, pd_total, mean_pd * 100),
  "Difference (HC − PD)" = diff_val,
  "Cohen's h" = sprintf("%.2f [%.2f, %.2f]", h_val, ci_low, ci_high)
) %>%
  gt() %>%
  fmt_number(columns = "Difference (HC − PD)", decimals = 3) %>%
  tab_header(title = "Group Comparison (Binary Outcome)")
